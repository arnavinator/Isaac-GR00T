...
- **Only ONE frame per chunk is ever consumed** (`video_delta_indices=[0]` → `_get_obs`
  ...
  sample on the LAST physics substep of a control step), and the vector-env observation
  concatenate, which indexes every observation-space key on every step, so skipped
  substeps must still carry placeholder frames or the worker dies with `KeyError`
  (`gym.make`'s `PassiveEnvChecker` only validates the first step). Note the collector venv pins gymnasium 0.29.1 (which forwards attributes

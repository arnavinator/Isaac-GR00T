"""Launch GR00T server with spectral temporal decomposition.

Same CLI interface as ``gr00t/eval/run_gr00t_server.py``.  Loads the standard
model, patches the action head's denoising loop to apply DCT-based frequency-
band velocity scaling, then starts the ZMQ policy server.
"""
...
@dataclass
class ServerConfig:
    """Configuration for the spectral denoising server."""
    ...
    gamma: float = 0.3
    """Guidance strength in [0, 1].  0 = baseline (no spectral scaling)."""

    energy_preserve: bool = True
    """Normalise velocity magnitude after spectral scaling."""


def main(config: ServerConfig):
    print("Starting GR00T server with SPECTRAL TEMPORAL DECOMPOSITION")
    print(f"  Model path: {config.model_path}")
    print(f"  Embodiment: {config.embodiment_tag}")
    print(f"  Device:     {config.device}")
    print(f"  Host:       {config.host}:{config.port}")
    print(f"  Params:     gamma={config.gamma}  energy_preserve={config.energy_preserve}")
    ...
    # Patch action head with spectral denoising
    patch_action_head(
        policy.model.action_head,
        gamma=config.gamma,
        energy_preserve=config.energy_preserve,
    )
    print("  Strategy:   spectral_temporal_decomposition (4 NFEs, DCT frequency scaling)")
